...
class Auth extends CI_Controller
{
		...
		if ($user) {
			...
			if ($user['is_active'] == 1) {
				...
				if (password_verify($pass, $user['password'])) {
					...
					$this->m_log->addLog('Login ke sistem', 'Auth', 'Login dari IP: ' . $this->input->ip_address());
	...
	{
		$this->m_log->addLog('Logout dari sistem', 'Auth', 'Sesi berakhir');
	...
	public function addUser()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!in_array($role_id, [1, 2, 3, 4])) {
				$role_id = 4; // Default to Warga jika tidak valid
			}
			$data = [
			...
			$this->m_log->addLog('Tambah user baru: ' . $new_username, 'Pengguna', 'Role ID: ' . $role_id);
	...
	public function editUser()
	{
		...
		$this->form_validation->set_rules('user', 'Full name', 'trim|required');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Nama lengkap wajib diisi!</div>');
			redirect('admin/user');
		} else {
			...
			if ($upload_img) {
				...
				if ($this->upload->do_upload('img')) {
					...
					if ($def_img != 'avatar.png') {
						unlink(FCPATH . 'frontend/assets/profil/' . $def_img);
					}
	...
	private function _restrictToSuperadmin()
	{
		$username = $this->session->userdata('username');
		if (!$username) {
			redirect('auth');
